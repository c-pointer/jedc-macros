...
ifnot ( is_defined ("HISTORY_Type") ) {
	typedef struct {
		_Values,	% Array of strings
		_first_idx,	% The oldest line
		_last_idx,	% The future line, should be considered empty on startup
		_next_idx,	% The current line, used for navigation in the history (prev, next)
		max_length,	% Currently unused; will be used if _Values will grow dynamically
...
private define mm_hist_next(self) {
	if (self.eohp()) return; % error ("End of list!");
...
private define mm_hist_prev(self) {
	if ( self.bohp() ) return; % error ("Top of list!");
	variable MX = length(self._Values);
...
private define mm_hist_do_store_value(self, s) {
	if ( (s == NULL) or (s == Null_String) ) return;
	variable MX = length(self._Values);
