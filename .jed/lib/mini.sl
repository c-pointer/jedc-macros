...
public define mini_use_history(histname) {
	if ( Mini_Max_History_Lines <  8 )	Mini_Max_History_Lines = 8;
	if ( Mini_Max_History_Lines > 64 )	Mini_Max_History_Lines = 64;
	if ( aHistory == NULL )				aHistory = Assoc_Type[HISTORY_Type];
	if ( histname == NULL )				histname = "default";
	else { 
		histname = strtrans(histname, "^[_0-9A-Za-z]", "_");
...
public define mini_exit_minibuffer() {
	bol_skip_white ();
	ifnot ( eolp() ) {
		try history.store_value(line_as_string());
		catch AnyError: ;
		}
	call("exit_mini");
	}
...
%	definekey("mini_complete", " ", mini);
	definekey("self_insert_cmd", " ", mini);
