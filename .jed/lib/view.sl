...
%      definekey(char, "error(help_message)", _stk_roll(2), mode);
   ...
   definekey("re_search_forward",                "/",      mode);
	definekey("re_search_forward",				 "s",   mode);
   definekey("repeat_search",                    "\\",     mode);
	definekey("help_message",                    "?",      mode);
	definekey("help_message",                    "h",	mode);
   ...
   definekey("search_forward",                   "f",	   mode);
	definekey("search_forward",                  "f",	mode);
	definekey("search_backward",                 "",	   mode);
	definekey("search_forward",                  "n",	   mode);
	definekey("search_backward",                 "N",	   mode);
   definekey("goto_line",                        "l",      mode); % g
	definekey("goto_line",                       "g",   mode);
   definekey("describe_mode",                    "h",      mode);
	definekey("bob",                              "g",      mode);
	definekey("eob; recenter(window_info('r'));", "G",      mode);
	definekey("bob",                             Key_Ctrl_Home,     mode);
	definekey("eob; recenter(window_info('r'));",Key_Ctrl_End,      mode);
	definekey("bob",                             Key_Ctrl_PgUp,     mode);
	definekey("eob; recenter(window_info('r'));",Key_Ctrl_PgDn,     mode);
	definekey("page_up",                         Key_PgUp,      mode);
	definekey("page_down",                       Key_PgDn,      mode);
	definekey("close_buffer",                    "Q",      mode);
	definekey("close_buffer",                    "q",      mode);
	definekey("close_buffer",                    "x",		mode);
   definekey("isearch_forward",                  "s",      mode);
%	definekey("isearch_forward",                 "s",      mode);
    definekey("close_and_insert_word",            "i",      mode);
   definekey("close_and_replace_word",           "r",      mode);
	definekey("cbrief_buffer_list_tui",			"\eb",		mode);	% Alt B -- Brief Manual: Buffer List (buffer managment list)
	definekey("cbrief_command",					Key_F10,	mode);	% Brief Manual: Execute command (like M-x emacs but with parameters)
	definekey("cbrief_command",					"\e=",		mode);	% Alt = -- Alternative key for console, just in case...
   _for (0, 9, 1)
...
public define view_mode()
{
   ...
   set_help_message(
"(Q)uit /:search (n)ext (N)ext backward (E)dit ?:help");
   run_mode_hooks(mode);
}
