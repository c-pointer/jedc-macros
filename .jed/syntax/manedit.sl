...
% Changelog
%
% 	----- - 2022/08/16: updated by nicholas christopoulos
%
%   1.0.1 - 2002/02/21:
...
() = define_keywords_n($0, ".BI.BR.EE.EX.HP.IB.IP.IR.LP.ME.MT.OP.PP.RB.RE.RI.RS.SB.SH.SM.SS.SY.TE.TH.TP.TS.UE.UN.UR.YS", 3, 1);
%}}}
...
%}}}
define manedit_insert_example_indent(start) { %{{{
  bol;
  if(start) insert(".EX\n");
  else insert(".EE\n");
}
...
define manedit_insert_hypertext(type) { %{{{
  ...
  else if(type == 3) insert(".MT " + read_mini("E-Mail:", "", "") + "\n[Real Name]\n.ME\n");
}
%}}}
...
define manedit_menu(menu) {  %{{{
  ...
  menu_append_popup(menu + ".&Indent", "E&xample indent");
  menu_append_item(menu + ".&Indent.E&xample indent", "&Start", "manedit_insert_example_indent(1)");
  menu_append_item(menu + ".&Indent.E&xample indent", "&End", "manedit_insert_example_indent(0)");
  %}}}
  %{{{ Lists
  ...
  menu_append_item(menu + ".&Hypertext", "&Mail", "manedit_insert_hypertext(3)");
