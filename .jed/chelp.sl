...
% escape sequences
% private variable escape_re=pcre_compile("\e\\[[0-9]+\\]");
% private define purge_escapes(str);
% private define purge_escapes(str) {
...
public define chelp() {
	...
	if ( flags & 0x01 ) % in whole window
		mesg(help_for_help_string);
	else
		mesg("Press F1 to change windows, F4 to close it, Ctrl+H for Jed's help_prefix");
	}
