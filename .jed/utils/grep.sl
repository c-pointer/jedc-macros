...
public define grep() % ([what], [path])
{
   ...
   % Prepare the output buffer
	variable _grep_buf = "*grep-output*";
	if ( bufferp(_grep_buf) )
		delbuf(_grep_buf);

	variable window_lines = 10;
   popup_buffer(_grep_buf, window_lines);
   setbuf_info("", dir, _grep_buf, 0);
   erase_buffer();
   set_buffer_modified_flag(0);
...
%%	Init
public define cbrief_grep() {
	variable argc = _NARGS, argv = __pop_list(argc);
	if ( argc == 1 )		grep(argv[0]);
	else if ( argc >= 2 )	grep(argv[0], argv[1]);
	else grep();
	}

public define cbrief_grep_replace() {
	variable argc = _NARGS, argv = __pop_list(argc);
	if ( argc == 1 )		grep->replace_cmd(argv[0]);
	else if ( argc >= 2 )	grep->replace_cmd(argv[0], argv[1]);
	else	grep->replace_cmd();
	}

static define init_unit_grep() {
	cbrief_cli_append("grep",         &cbrief_grep,         6,
"grep [text [files]]\n\
	Runs grep inside of grep and puts the results in a new buffer.\
	To open a file on a result line, go to the line press ENTER or double click on it.\
\n\
	To replace text across the matches (both, grep output buffer and source\n\
	files), use the keybinding (^Ol) for the replace_cmd() or the Mode menu." );
	cbrief_cli_append("grep_replace", &cbrief_grep_replace, 6,
"grep_replace\n\
	It is part of grep command. Better use it from menu or binded key." );
	cbrief_setkey("grep", "^Og");
	cbrief_setkey("grep->replace_cmd", "^Ol");
	cbrief_menu("Global.S&ystem", "&Grep command", "grep");
	cbrief_menu("Global.S&ystem", "Grep rep&lace command", "grep->replace_cmd");
	}
init_unit_grep();
