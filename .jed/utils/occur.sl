...
. "occur->occur_goto_line" "\n"
. "occur->occur_goto_line" "\r"
. "occur->next" "n"
. "occur->prev" "p"
. "occur->scroll" " "
...
%%	Init
static define init_unit() {
	cbrief_cli_append("occur",  &occur,  2,
"occur\n\
	This function may be used to search for all occurrences of a string in\
	the current buffer.  Without arguments, it prompts for a string to search\
	for, defaulting to the word at point.  It creates a separate buffer called\
	‹*occur*› and associates a keymap called ‹Occur› with the new buffer.\
	In this buffer, the [g] and [enter] key may be used to go\
	to the line described by the match.\
	Each line is displayed with ‹nlines› lines before and after.\
	‹Nlines› defaults to ‹Occur_Context›. Interactively it is the prefix arg." );
	cbrief_cli_append("moccur", &moccur, 2,
"moccur\n\
	‘occur’ multiple buffers." );
%	cbrief_setkey("occur",  Key_Alt_F11);
%	cbrief_setkey("moccur", Key_Ctrl_F11);
%	cbrief_menu("Global.&Search", "Find occurances", "occur");
	cbrief_menu("Global.&Search", "Find o&ccurances", "moccur");
	}
