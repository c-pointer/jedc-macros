...
%
% 2022-10-14 - Nicholas Christopoulos -
% 	patched to support the new GROFF_SGR != 0 of groff
%
...
%\seealso{unix_man}
%!%-
custom_variable("Man_Clean_Headers", 1);
...
%\seealso{unix_man}
%!%-
custom_variable("Man_Use_Extensions", 0);
...
static variable ecma_bold_on  = "\033\[1m";
static variable ecma_ital_on  = "\033\[3m";
static variable ecma_ul_on    = "\033\[4m";
static variable ecma_bold_off = "\033\[22m";
static variable ecma_ital_off = "\033\[23m";
static variable ecma_ul_off   = "\033\[24m";

static variable underline_marker = sprintf("\e[%d]", color_number("underline"));
static variable bold_marker      = sprintf("\e[%d]", color_number("bold"));
static variable italic_marker    = sprintf("\e[%d]", color_number("italic"));
static variable reset_marker     = sprintf("\e[%d]", color_number("normal"));

static define man_2022_esc() {
%	bob(); replace("\033",  "@");
	bob(); replace(ecma_bold_on,  bold_marker);
	bob(); replace(ecma_bold_off, reset_marker);
	bob(); replace(ecma_ital_on,  italic_marker);
	bob(); replace(ecma_ital_off, reset_marker);
	bob(); replace(ecma_ul_on,    underline_marker);
	bob(); replace(ecma_ul_off,   reset_marker);
	}
...
static define parse_ref(word) {
	if ( pcre_exec(man_re, word) ) { % e.g. man(Nxxx)
		variable sec, page;
...
static define make_ref(subj) {
    ...
    { case 3 && (subj[0] == "-e "): return sprintf("%s(%s)", subj[2], subj[1]); }
    { case 3 && (subj[0] == "-s "): return sprintf("%s(%s)", subj[2], subj[1]); }
	{ return subj[-1]; }
	}
%}}}
...
% both
   man_cmd = sprintf("MAN_KEEP_FORMATTING=1 MANWIDTH=%d man %s -O width=%d 2> /dev/null", SCREEN_WIDTH, subj, SCREEN_WIDTH-4);
#endif
	...
	if ( return_status && bobp() && eobp() ) {
		...
		if ( 16 == return_status )
			...
			throw RunTimeError, "man ($man_cmd) returned an error (return status $return_status)"$;
     	}
   	man_clean_manpage;
	}
%}}}
...
static variable numbers = "0123456789abcdefghijklmnopqrstwuvxyz";
